import { existsSync, lstatSync, readdirSync, renameSync, readFileSync, writeFileSync } from 'fs';
import { createInterface } from 'readline';
import { basename, resolve, format, dirname, extname } from 'path';
...
function parseArgs(args) {
	...
	if (unknownArgs.length) {
		...
		unknownArgs.forEach(arg => {
			...
			const exists = existsSync(arg);
			if (exists) {
				vlog(4, `Checking if ${arg} is a directory`);
				const isDir = lstatSync(arg).isDirectory();
				if (isDir) {
...
const convertDirectory = function (path) {
	...
	let currentDirectory = readdirSync(path, { withFileTypes: true });

	vlog(4, `Organizing directory contents`);
	for (let i = 0; i < currentDirectory.length; i++) {
		let currentPath = format({
			dir: path,
	...
	for (let i = 0; i < files.length; i++) {
		...
		if (extname(file) === '.md') {
			...
			writeFileSync(file, correctedFileContents.content, 'utf8');
